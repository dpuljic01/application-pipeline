# Job Dossier — Compressed Build Plan
...
- ✅ Day 4 — Company Model + Relationships (2026-09-11): `Company` model (user-scoped, same as everything else), repository/service/router following the Day 2 layering pattern, `GET /companies/{id}/applications`. Deviations: `Application.company` (free-text string) was kept as-is rather than replaced — `company_id` was added *alongside* it as a nullable FK, matching the plan's own "nullable for now" phrasing; the relationship is named `linked_company` since `company` was already the string column's name. Case-insensitive per-user uniqueness is a real DB-level functional unique index on `lower(name)`, not just an app-level check — makes `ApplicationService`'s auto-create-or-link-by-name path race-safe. Delete-with-applications is a domain error (`CompanyHasApplications`) mapped to 409, with `ondelete="RESTRICT"` on the FK as a DB-level backstop. `contacts` stretch table not built. All 13 new tests pass against a real migrated Postgres (not just the transactional-rollback fixture logic — actually ran `alembic upgrade head` live).
- ✅ Day 7 — Route protection (`get_current_user_id()` stub replaced with real `get_current_user`, verified against live Cognito); global exception handlers added (2026-09-17): `DomainError`/`Exception` handlers in `backend/app/main.py`, defense-in-depth only (routes still map their own errors explicitly per CLAUDE.md's table, which now also documents the two error types added since — `JDNotParsed`/`JDParseError`/`MatchingError`/`FollowUpGenerationError` from Days 9-11 weren't in the table until now). JWKS-unreachable now correctly 503 (was 500) and no longer leaks the underlying httpx exception text to the client. `core/logging.py` (previously an empty stub) now does minimal stdlib `logging.basicConfig` setup so the catch-all handler has somewhere to log unexpected errors server-side. Tests: JWKS-failure → 503 (mocked transport failure, first test coverage `core/security/` has ever had), plus the two handlers unit-tested directly against a fabricated `Request` (TestClient's default debug-reraise behavior for unhandled exceptions makes testing "what does the client actually receive" awkward through the real client).
...
- 🟨 Day 8 — Provider-Agnostic LLM Client + Gemini Setup (2026-09-01): `app/integrations/llm/` built — `base.py` (`LLMProvider` protocol, `LLMResponse`, `DisabledLLMProvider`), `gemini_provider.py`, `anthropic_provider.py`, `factory.py` (`get_llm_provider()` reads `LLM_ENABLED`/`LLM_PROVIDER`), `pricing.py`, `cost_tracker.py`; `llm_usage` table + migration (`LLMProviderName` enum, same create/drop-type pattern as the existing enums). Verified live against real Gemini API. **Deviations found by testing against the live API, not guessable from training data:** `gemini-2.5-flash` is retired for new API keys — the API's own 404 pointed to `gemini-3.6-flash`, now the default (pricing updated to $0.75/$3.75 per 1M, promotional through 2026-12-31). Gemini 3.6 Flash has extended thinking on by default and cannot fully disable it (`thinking_budget=0` → 400 `INVALID_ARGUMENT`) — `thinking_level=MINIMAL` works and is now set explicitly, otherwise `max_output_tokens` gets silently consumed by thinking tokens with empty visible output (`finish_reason=MAX_TOKENS`, `thoughts_token_count>0`, `text=None` — a real footgun, not obvious from the response unless you inspect `usage_metadata`). `automatic_function_calling` disabled explicitly to silence SDK warning noise (no tools used here). **Design deviation from CLAUDE.md's "only services call db.commit()"**: `CostTracker` commits its own writes to `llm_usage` — deliberate, since a usage record is an audit fact about a call that already happened, not part of the caller's business transaction. Not yet run through `/architecture-review`. Anthropic provider verified structurally (factory dispatch, unit tests) but not against the live API — no `ANTHROPIC_API_KEY` set yet. Still open: Day 9's JD parser is what actually exercises structured output (`response_json_schema`) and the cost tracker from a real service/route.
- ✅ Day 9 — Job Description Parser (2026-09-16): `ParsedJobDescription` schema (`api/schemas/jd_parse.py`, `Literal` types for `seniority_assessed`/`salary_confidence` so the JSON schema constrains the LLM to valid values, every field required-but-nullable rather than optional-to-omit — more reliable for structured output across providers); `LLMProvider.complete()` extended with `response_schema: type[BaseModel] | None`; Gemini via native `response_schema`, Anthropic via forced tool-use (no JSON mode on this SDK version) — both verified live. **Real deviations found only by testing live, not guessable from training data:** Anthropic's `messages.create` no longer accepts `temperature` at all in SDK 1.2.0 (dropped from `AnthropicProvider`, silently, Protocol still accepts it for interface parity); a workspace-unscoped API key 400s on every call ("not scoped to a workspace") — fixed by rescoping the key in the Console; Anthropic's newer native `output_config.format` JSON-schema mode exists but requires that same workspace scoping in a way tool-use doesn't — stuck with tool-use. Anthropic prompt caching wired (`cache_control` on the system prompt block, cache-aware cost accounting added to `pricing.py`) but empirically inactive on real calls up to ~3800 tokens — root cause: Haiku 4.5's cache-write minimum is 4096 tokens, higher than Sonnet's 1024. Decided *not* to pad the prompt to clear it or switch to Sonnet: real per-call math (using measured completion-token counts) showed Haiku uncached beats Sonnet cached for this output-heavy workload — Sonnet's 2x output price outweighs the input-side cache savings. `services/jd_parser.py` — system prompt has a deliberate DACH-market judgment call: "fliessend/verhandlungssicher German" stated as a hard requirement is flagged as a red flag, "gute/sehr gute Deutschkenntnisse" is not, since that phrasing is routinely overstated in Swiss/DACH postings — verified live with both phrasings to confirm the distinction actually holds. `JDParseError` added to `domain/errors.py`, mapped to 502 in the route (matches the JWKS-unreachable-503 convention from Day 7). `applications.parsed_jd` JSONB column (first raw-JSON column in this schema — deliberate, it's a display cache of one LLM call, not something ever queried by individual field) + migration `6975cbcc35a7`. `POST /applications/{id}/parse-jd` verified end-to-end against the real local DB (service method directly, not just import-checked) — confirmed `NotFound` → 404 path and a real Gemini call persisting to `parsed_jd` and surviving a DB refresh. Not yet done: Day 9b's actual public route/page (the shared-engine design is in place and this is what it's designed to plug into, but the unauthenticated endpoint and `/analyze` frontend page themselves aren't built).
- 🟨 Day 10 — Profile Matching + Scoring (2026-09-16): diverges from this day's original spec below — `domain/profile.py` as a hardcoded dataclass was rejected in favor of a real, user-editable `Profile` DB resource (`db/models/profile.py`, full CRUD via `GET`/`PUT /profile`), since the actual requirement was a Setup/Profile page, not a static file. Deliberately ships with **no seed/default personal data anywhere in code** — a freshly auto-created profile is empty (`skills: []`, `years_experience: null`, etc.); salary expectations, years of experience, and home address are private data that doesn't belong compiled into a portfolio repo, so it only ever lives in the DB, entered through the page. Rule-based scorer (`services/matcher.py`) is 5 components summing to 0–100 — skills (0–40), seniority (0–20, handles accepting *both* mid and senior via a `target_seniorities` list rather than one enum value), language (0–15), salary (0–15, a graduated curve between a floor and an ideal rather than a hard cutoff, regex-parsed from the LLM's free-text `salary_range`), remote/policy (0–10) — with missing profile data treated as neutral, not a penalty, so scoring degrades gracefully before the profile is fully filled in. **A commute-distance heuristic (hardcoded city→minutes lookup table) was designed then deliberately cut before implementation** — it baked one person's home address into backend code, and per its own scoring rules would have rarely changed the result anyway (remote/hybrid and unrecognized cities both always scored full marks). Replaced with a cheaper, more honest approach: the LLM-qualitative pass (`generate_match_insights`, new `MatchInsights` schema, same forced-structured-output pattern as Day 9) is hydrated with the profile's free-text `home_location` and the posting's location, and asked to flag commute concerns narratively using its own knowledge of geography — no hardcoded table to maintain, works for any home location. `match_score`/`match_details` JSONB columns added to `applications`, migration chained after Day 9's. Backend fully done and tested (99 tests passing, up from 57 at the start of the day — Profile CRUD, the rule-based scorer's edge cases, and the scoring API's 404/409/502 paths all covered with a `FakeLLMProvider` test double). Frontend also built — `/profile` page (tag-input for skills, repeatable language rows, seniority toggle-buttons, salary floor/ideal, home location), `MatchScorePanel` on the application detail page, a Match column on the dashboard table/card, `PostingScoreGauge` generalized with an optional caption to serve both posting-quality and match scores — type-checks, lints, and builds clean. **Not yet done:** a real logged-in browser click-through (needs live Cognito credentials this session doesn't have) — verified instead via the backend's full test suite plus a pre-login dev-server smoke check (all routes serve without a runtime error). `/architecture-review` not yet run.
- 🟨 Day 11 — Follow-Up Nudge + Email Generator (2026-09-17, simplified on request): the stale-application nudge (originally cut with the Days 11-13 background-worker/reminder-rules infra in "What Was Cut") is rebuilt as a **frontend-only computed flag** — `needsFollowUp()` in `frontend/lib/followup.ts` derives staleness from `stage` + `stage_changed_at`, both already fetched with every application. No new column, no cron job, no `ReminderRule`. Thresholds: 14 days at `APPLIED`, 7 at `INTERVIEW` (the two stages where silence actually calls for a follow-up; `SAVED`/`OFFER`/terminal stages don't). Paired with the actual Day 11 spec below, simplified per explicit direction: **one email, not the spec's two formal/conversational variants**, and cached directly on the application as `generated_followup` JSONB (same "one cached LLM artifact" pattern as `parsed_jd`/`match_details`, not the spec's separate `generated_emails` table with a `used` flag). System prompt (`services/followup_generator.py`) explicitly bans em dashes and generic AI-isms, stays under ~120 words, and must reference the real company/role — a deliberate reaction to em dashes reading as an obvious AI tell in something meant to go to a real hiring contact; this is prompt-level and verified live, not something a mocked-provider unit test can check. New `POST /applications/{id}/generate-followup`, `FollowUpGenerationError` → 502, same shape as the two existing LLM routes. Backend fully tested (107 passing, up from 99). Frontend: a `FollowUpBadge` next to the stage badge on the dashboard table/card when stale (amber `--primary`, deliberately unused by any stage color per `stage-badge.tsx`'s own comment), and a `FollowUpPanel` on the detail page with an optional context field, a copy-to-clipboard action, and a callout when the open application is stale. Type-checks, lints, and builds clean. **Not yet done:** live logged-in verification (same live-credential limitation as Day 10) — can't confirm from here whether the model actually honors the no-em-dash instruction in practice, only that the prompt asks for it.
- ✅ Architecture review, Days 9-11 (2026-09-17) — `/architecture-review`, Socratic pass over the JD parser, matcher, and follow-up generator plus the same day's error-handling work. Two real findings, both fixed: (1) **services were importing Read-shaped Pydantic schemas from `api/schemas/`** (`ParsedJobDescription`, `MatchInsights`, `FollowUpEmail`, `ProfileRead`) — not literally forbidden by CLAUDE.md's letter (only `*Create`/`*Update` are named), but the same coupling problem in spirit: business logic reaching into API-contract types. Fixed by relocating `ParsedJobDescription` into `services/jd_parser.py`, `MatchInsights`+new `MatchDetails` into `services/matcher.py`, and `FollowUpEmail` into `services/followup_generator.py` — each schema now lives beside the one function whose LLM call actually defines its shape, and `api/schemas/application.py` imports them for its response fields instead of the reverse. `api/schemas/match.py` deleted (nothing genuinely API-only was in it). `matcher.py` also stopped accepting `ProfileRead` and converting it internally — it now takes the ORM `Profile` object directly throughout, the same way `ApplicationService` never converts `Application` to a Pydantic type internally; `Profile.languages` (JSONB) entries are accessed as plain dicts, not `LanguageEntry` objects. (2) **`ApplicationService.update_application`/`CompanyService.update_company`/`CompanyService.create_company`/`ProfileService.update_profile` all imported `*Create`/`*Update` schemas directly** — a real, literal violation of CLAUDE.md's explicit rule, present since Day 2/4 and inherited by Day 10's `ProfileService`, not something Days 9-11 introduced but caught while reviewing them. Fixed across all three services: routes now call `payload.model_dump(exclude_unset=True)` (plus the `AnyHttpUrl`→`str` coercions that used to happen service-side) and pass a plain `dict`; every service now takes `data: dict` or fully unpacked keyword args instead of the schema object. `ApplicationRead.match_details`/`.generated_followup` also upgraded from bare `dict` to proper `MatchDetails`/`FollowUpEmail` types as part of the same cleanup — real OpenAPI schema documentation for both fields now instead of an opaque blob. Also made keyword-only argument usage consistent across `services/jd_parser.py`/`matcher.py` (was previously mixed: some multi-arg helpers positional, some keyword-only). 114 tests passing throughout (no regressions), full app import + route registration sanity-checked directly. Separately fixed in the same session: JWKS-unreachable now correctly 503 not 500 (see Day 7's entry above), and global `DomainError`/`Exception` handlers added to `main.py` as defense-in-depth — confirmed this does *not* contradict CLAUDE.md's "never let domain errors propagate to FastAPI's default handler" rule, since a registered custom handler producing the same `{"detail": ...}` shape every route already returns is categorically different from Starlette's true unhandled-exception fallback (a raw traceback or leaky generic 500).
- ✅ Live deploy (2026-09-16/17, supersedes the plan below) — actual shipped path ended up simpler than either the AWS or the original Vercel-fronting-AWS plan: **backend on Render** (`render.yaml` "Track A", free tier, Docker runtime) with an external Neon Postgres (Render's own Postgres free tier expires after 30 days), **frontend on Vercel**, both on custom `puljic.ch` subdomains — `api.puljic.ch` (backend, CNAME to Render, TLS auto-issued by Render) and `jobs.puljic.ch` (frontend). `NEXT_PUBLIC_API_BASE_URL` and `CORS_ORIGINS` set accordingly on each platform. **Two production incidents so far, both the same root cause**: Render doesn't auto-run migrations on deploy, so both the Day 9 (`parsed_jd`) and Day 10 (`match_score`/`match_details`) migrations shipped code before schema, 500ing `GET /applications` in production until `alembic upgrade head` was run manually against Neon. Not yet fixed structurally — still needs a Render pre-deploy command (or a personal checklist step) so this can't recur with future migrations. AWS (Days 13-14's Terraform: VPC/RDS/ECS/ALB) still exists as code and was verified working before being `terraform destroy`'d per the cost-management plan, but is not the live path — reviving it needs an ACM cert + HTTPS listener (not built) and is explicitly meant to be spun up on-demand for a specific interview/demo, not run 24/7 (NAT Gateway + ALB run ~$50/mo). Out of scope so far either way: GitHub Actions → ECS (Day 16), S3 docs (Day 15), Route53, Multi-AZ/autoscaling.
...
### Day 9b — Public JD Analyzer (Business Experiment) 🆕

**Goal:** A free, unauthenticated "paste a job posting, get an instant breakdown" tool, built on top of Day 9's parser, for distribution on LinkedIn and Swiss/DACH job-seeker groups.

**Context:** This is outside the original 18-day interview-prep scope — it's a monetization experiment, not a portfolio checkbox. Researched the competitive landscape first (see `DECISIONS.md` once written): several free tools already do generic JD-analysis (AnythingText, Qarera) and ghost-job detection (ScamShield, VantageCV, Ghost Job Hunter) — all US/generic-market, none localized, most are lead-gen for a bigger paid product rather than standalone businesses. That rules out charging for the analyzer itself head-on. The differentiated angle: DACH localization (CHF salary bands, German-language postings, Swiss-specific red flags like unpaid trial days or permit bait-and-switch) that no incumbent targets — and the monetizable layer is "save this to your tracker," not the analysis itself.

**Design decision:** one schema, one parser, two callers. `services/jd_parser.py` (Day 9) is the shared engine; this day adds a second, thin route that calls the same service unauthenticated instead of building a parallel parser. Real production pattern — one domain service, multiple API entry points with different auth/rate-limit policies.

**Tasks:**
1. Extend `ParsedJobDescription` (Day 9) with the extra fields the public tool needs: `seniority_claimed` vs `seniority_assessed` (title-inflation signal), `salary_confidence`, `missing_info`, one-line `summary`. Costs one extra field or two in the same LLM call — not a second call.
2. `POST /api/jd/analyze` — unauthenticated, `{ text: str }` in, calls `services/jd_parser.py` directly (same function Day 9's authenticated route calls).
3. Per-IP daily rate limit (in-memory counter keyed by IP+date — good enough for a single Render instance; controls LLM cost with zero new infra).
4. Frontend `/analyze` page: textarea → results card, same design system as the rest of the app. Logged-in users get a "Save to tracker" button that prefills `POST /applications` (already built); logged-out users see a "sign up to save" CTA into the existing signup flow.

**Acceptance Criteria:**
- Same parser code powers both `/applications/{id}/parse-jd` (Day 9) and `/api/jd/analyze` (this day) — no duplicated extraction logic.
- Anonymous users are capped by a per-IP daily limit; exceeding it returns a clear 429, not a crash or a silent free-for-all.
- A logged-in user can go from pasting a JD to a tracked application in one click.

**Learning Checkpoint:** Why serve two different trust levels (authenticated vs. anonymous) from the same underlying service instead of two separate implementations? Where does the rate limit actually need to live if this ever moves behind a real load balancer with more than one instance?
